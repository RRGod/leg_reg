...
def train_mt(label_loader, unlabel_loader, model, model_teacher, criterions, optimizer, epoch, args, ema_const=0.95, weight_mt=8.0):
    nEpochs, num_classes, weight_l1, print_freq = args.nEpochs,args.num_classes,args.weight_l1,args.print_freq
    ...
    for i in range(len_iter):
        ...
        weight_cl = cal_consistency_weight(global_step, end_ep=(nEpochs//2)*len_iter, end_w=1.0)
        ...
        input_var = Variable(input, requires_grad=True).to(device)
        input1_var = Variable(input1, requires_grad=True).to(device)
        input_ul_var = Variable(input_ul, requires_grad=True).to(device)
        input1_ul_var = Variable(input1_ul, requires_grad=True).to(device)
...
def validate(val_loader, model, criterions, args, mode = 'valid'):
    ...
    with torch.no_grad():
        for i,(input, target,input1) in enumerate(val_loader):
            sl = input.shape
            batch_size = sl[0]
            target = target.cuda(non_blocking=True)
            input_var = Variable(input, requires_grad=True).to(device)
           ...
           # one_hot =  one_hot = torch.zeros(np.array(batch_size, 7, device=torch.device('cuda:0')).scatter_(1, target_var, 1))
            loss = criterion(output, target_var.long()) / float(batch_size)
    ...
    args.logger.info(' ****** Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Loss {loss.avg:.3f} '
